...
export function HomeScreen({ onJoined }) {
  ...
  const handleJoin = async () => {
    ...
    const { data: game, error: gameErr } = await supabase
      .from('mv_games').select('*').eq('code', upperCode).single()
    ...
    const { data: existing } = await supabase
      .from('mv_players').select('*').eq('id', playerId).eq('game_id', game.id).single()
    if (existing) {
    ...
    <div className="screen flex flex-col">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center flex-1 px-6 py-12 gap-10">
        ...
        <div className="flex flex-col items-center gap-3 animate-moon-rise">
          ...
          <h1 className="font-display font-black text-5xl text-gold text-shadow-gold tracking-wider text-center">
            Le Village
          </h1>
          ...
          <p className="text-parchment-dim text-xs font-body tracking-widest uppercase text-center">
            Grasse · 4 à 12 joueurs · Loup-Garou
          </p>
        </div>
        ...
        {!mode ? (
          <div className="w-full max-w-sm flex flex-col gap-4 animate-fade-up">
            <Button variant="primary" className="w-full text-base py-5" icon="🏰"
              onClick={() => setMode('create')}>
              Créer une partie
            </Button>
            <Button variant="ghost" className="w-full text-base py-5" icon="🚪"
          ...
          <div className="w-full max-w-sm card-dark p-6 flex flex-col gap-5 animate-fade-up">
            ...
            <div className="flex flex-col gap-3">
              <input className="input-dark" placeholder="Ton prénom"
                value={name} onChange={e => setName(e.target.value)} maxLength={20} autoComplete="off" />
              {mode === 'join' && (
                <input
            ...
            {error && (
              <p className="text-blood-light text-sm font-body text-center animate-shake">{error}</p>
            )}
            <Button variant="primary" className="w-full"
              onClick={mode === 'create' ? handleCreate : handleJoin} disabled={loading}>
