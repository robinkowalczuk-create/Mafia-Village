...
export function RoleRevealScreen({ game, currentPlayer, players = [] }) {
  ...
  // Vérifier si le Voleur est dans la partie
  const thiefInGame = players.some(p => p.role === 'thief')

  // Quand tous ont vu leur carte → aller au THIEF_TURN si voleur, sinon NIGHT
  useEffect(() => {
    ...
    const checkAllReady = async () => {
      ...
      if (allReady) {
        const nextPhase = thiefInGame ? PHASES.THIEF_TURN : PHASES.NIGHT
    ...
    checkAllReady()
  }, [players, game?.id, game?.current_phase, thiefInGame])
  ...
  const handleFlip = () => {
    ...
    setTimeout(() => {
      setRevealed(true)
      // Pas de son — évite de trahir le rôle
    }, 400)
  }
  ...
  return (
    <div className="screen flex flex-col">
      ...
      <div className="absolute inset-0 opacity-20 transition-opacity duration-1000"
        style={{ background: revealed ? `radial-gradient(ellipse at center, ${role.color}40 0%, transparent 70%)` : 'transparent' }} />

      <div className="relative z-10 flex flex-col items-center justify-between flex-1 px-6 py-10">

        <div className="text-center animate-fade-in">
          <p className="text-parchment-dim text-xs uppercase tracking-widest font-body mb-2">Distribution secrète</p>
          <h1 className="font-display font-black text-2xl text-gold">Votre rôle vous attend</h1>
          <p className="text-parchment-dim text-sm mt-2 font-body">Cachez votre écran des autres joueurs</p>
        </div>
          ...
          <div className={`role-card-inner w-full h-full ${flipped ? 'flipped' : ''}`}>
            <div className="role-card-front">
              <div className="w-64 h-96 rounded-3xl border-2 border-gold/30 flex flex-col items-center justify-center gap-6 cursor-pointer active:scale-95 transition-transform card-glow-gold animate-float"
                ...
                <p className="font-display text-gold/60 text-sm tracking-wider uppercase">Appuyez pour révéler</p>
                <div className="flex gap-1">
                  {[0,1,2].map(i => (
                    <div key={i} className="w-1 h-1 rounded-full bg-gold/40 animate-pulse" style={{ animationDelay: `${i * 0.2}s` }} />
                  ))}
                </div>
              </div>
            </div>
            <div className="role-card-back">
              <div className={`w-64 h-96 rounded-3xl border-2 overflow-hidden flex flex-col items-center justify-center gap-4 ${revealed ? 'animate-glow-in' : ''}`}
                style={{
                ...
                <RoleImage role={role} className="w-36 h-36" />
        ...
        <div className="w-full max-w-sm flex flex-col gap-4">
          {revealed && (
            <div className="card-dark p-4 animate-fade-up" style={{ borderColor: `${role.color}30` }}>
              <p className="text-xs uppercase tracking-wider font-body mb-2" style={{ color: role.colorLight }}>Vos pouvoirs</p>
              {role.powers.length > 0 ? (
                <ul className="flex flex-col gap-1">
                  {role.powers.map((power, i) => (
                    <li key={i} className="text-parchment-dim text-xs font-body flex gap-2">
                      <span style={{ color: role.color }}>›</span>{power}
                    </li>
                  ))}
                </ul>
              ) : (
                <p className="text-parchment-dim text-xs font-body">Votez le jour venu. C'est votre seule arme.</p>
              )}
            </div>
          )}
          ...
          {ready && (
            <div className="card-dark p-4 text-center border-forest/30 animate-fade-in">
              <p className="text-forest-light text-sm font-body">✓ Prêt · En attente des autres joueurs...</p>
              <p className="text-parchment-dim text-xs font-body mt-1">{readyCount} / {players.length} prêts</p>
            </div>
          )}
        </div>
      </div>
